...
from .serializers import RegisterSerializer,SetRoleSerializer,MyTokenObtainPairSerializer,ProfileImageSerializer,UserProfileSerializer
...
@extend_schema_view(
    ...
    update=extend_schema(
        description="Update only your own profile"
    ),
...
class UserProfileViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['patch'], serializer_class=ProfileImageSerializer)
    def upload_image(self, request, pk=None):
        profile = self.get_object()
        if profile.user != request.user:
            return Response({"detail": "You cannot upload image for another user."}, status=403)
